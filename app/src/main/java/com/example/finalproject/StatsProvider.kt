...
class StatsProvider : ContentProvider() {
    companion object {
        val PROVIDER_NAME = "com.example.finalproject.StatsProvider"
        val URL = "content://" + PROVIDER_NAME + "/players"
        val CONTENT_URI = Uri.parse(URL)
        val _ID = "_id"
        val NAME = "name"
        val SCORE = "score"
        private val PLAYERS_PROJECTION_MAP: HashMap<String, String>? = null
        val PLAYERS = 1
        val PLAYER_ID = 2
        val uriMatcher: UriMatcher? = null
        val DATABASE_NAME = "Game"
        val PLAYERS_TABLE_NAME = "players"
        val DATABASE_VERSION = 1
        val CREATE_DB_TABLE = " CREATE TABLE " + PLAYERS_TABLE_NAME +        " (_id INTEGER PRIMARY KEY AUTOINCREMENT, " + " name TEXT NOT NULL, " +
                " score TEXT NOT NULL);"
    }
    ...
    override fun update(
        ...
        var count = 0
        when (uriMatcher!!.match(uri)) {
            PLAYERS -> count = db!!.update(
                PLAYERS_TABLE_NAME, values, selection,
                selectionArgs
            )
            PLAYER_ID -> count = db!!.update(
                PLAYERS_TABLE_NAME,
                values,
    ...
    override fun getType(uri: Uri): String? {
        when (uriMatcher!!.match(uri)) {
            PLAYERS -> return "vnd.android.cursor.dir/vnd.example.players"
            PLAYER_ID -> return "vnd.android.cursor.item/vnd.example.players"
